...
let manualScrollTimers = []; // To store setTimeout IDs for manual scroll overrides
...
function toggleAutoScroll(enable) {
  views.forEach((v, i) => {
    ...
    if (manualScrollTimers[i]) {
      clearTimeout(manualScrollTimers[i]);
      manualScrollTimers[i] = null;
    }
...
function create() {
  ...
  ipcMain.on('manual-scroll', (_, idx) => {
    ...
    if (manualScrollTimers[idx]) {
      clearTimeout(manualScrollTimers[idx]);
    }
    // Set a new timeout to resume auto-scroll after 5 seconds
    manualScrollTimers[idx] = setTimeout(() => {
